...
public class FirmwareUpdateActivity extends AppCompatActivity {
    ...
    private void initListener() {
        ...
        BleReceiverHelper.registerBleReceiver(this,bleReceiver);
    }
    ...
    private BluetoothAdapter.LeScanCallback mLEScanCallback = new BluetoothAdapter.LeScanCallback() {
        ...
        public void onLeScan(final BluetoothDevice device, final int rssi, final byte[] scanRecord) {
            ...
            KLog.i("----------"+address+"---------"+name);
            //dialog
            if (isDialog() && device.getAddress().equals(PrefUtil.getString(context,BaseActionUtils.ACTION_DEVICE_ADDRESS))) {
                ...
                Intent updateIntent = new Intent(context, NewDfuService.class);
                updateIntent.putExtra(DfuService.EXTRA_DEVICE_ADDRESS, address);
                KLog.e("dialog DFU　search MAC ：" + address);
...
//								updateIntent.putExtra(DfuService.EXTRA_FILE_URI, uri);
            ...
            //nordic
            if (ScannerServiceParser.decodeDeviceAdvData(scanRecord, BaseActionUtils.NODIC_UPDATE_SERVICE) ||name.contains("DFU")) {
                if (judgeRepeat(address)) {
                    return;
                }
                mHandler.removeCallbacks(mScanTimeOutRunnable);
...
//        newMac = oneMac + last;
    ...
    private void writeDfuCmd() {
        if (BluetoothUtil.isConnected()) {
            if (!isDialog()) {
                ...
                byte[] data = SuperBleSDK.getSDKSendBluetoothCmdImpl(this).setUpgrade();
                BleWriteDataTask task = new BleWriteDataTask(context, data, BleWriteDataTask.TYPE_UPDATE);
    ...
    private BluetoothCallbackReceiver bleReceiver= new  BluetoothCallbackReceiver(){
        ...
        public void onCommonSend(byte[] data) {
            ...
            if (data.length == 4 && data[0] == (byte) 0x21 && data[1] == (byte) 0xFF && data[2] == (byte) 0x03 && data[3] == (byte) 0x00) {
                KLog.e("写入DFU指令成功");
                //成功写入dfu指令
                mHandler.removeCallbacks(mWriteDFUTimeoutRunnable);
        ...
        public void onCmdReceiver(byte[] data) {
            ...
            if (data.length == 5 && data[0] == 0x06 && data[1] == 0x00 && data[2] == (byte) 0x81 && data[3] == 0x01 && data[4] == 0x00) {
                KLog.e("写入DFU指令成功");
                //成功写入dfu指令
                mHandler.removeCallbacks(mWriteDFUTimeoutRunnable);
                mHandler.postDelayed(new Runnable() {
