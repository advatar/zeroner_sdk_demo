...
//        case 2: return MUTE;
        ...
        if (hisDataCase == CASE_INDEX_TABLE) {
            fixedThreadPool.execute(new Runnable() {
                ...
                public void run() {
                    ...
                    ProtoBufSync.getInstance().syncDetailData(context,index_80s);
                }
            });
    ...
    private synchronized static void finishHealth(int progress) {
...
//        int sum = year * 380 + month * 31 + day;
//        int dateSum = date[0] * 380 + date[1] * 31 + date[2];
//        if (dateSum == 0) {
//            date[0] = year;
//            date[1] = month;
//            date[2] = day;
...
//            if (dateSum != sum) {
//                //同步完一天的数据
//                Log.e("time_chanege", date[0] + "-" + date[1] + "-" + date[2]);
//
...
//                DateUtil d = new DateUtil(date[0], date[1], date[2]);
//                if (d.isToday()) {
//                    byte[] data = ProtoBufSendBluetoothCmdImpl.getInstance().getRealHealthData();
//                    BackgroundThreadManager.getInstance().addWriteData(ContextUtil.app, data);
